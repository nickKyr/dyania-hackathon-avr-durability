...
[`results.md`](results.md) holds the calibration table and the degradation ladder. It is
regenerated by `scripts/05_report_synthetic.py` and stamped with the commit it ran against; the
extract row at the foot of the ladder is measured by `scripts/09_real_extract_rung.py`. The real
extract reaches the models only through `notebooks/02_preprocessing.ipynb`.
...
```bash
uv run python -m pytest synthetic/tests -q     # from notebooks/, as above
```
