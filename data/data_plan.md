...
prototype extract. Section 1 describes the data a real deployment would need, and section 5 the
synthetic cohort and what it was measured against. Companion documents:
...
|---|---|---|
| Serial follow-up echoes are available per patient | **Fails.** 143 mean-gradient mentions across 54–59 patients, but only **one patient** has values in more than one note or year in the rule-based pass (4 after the reconciled preprocessing, §5). | High — removes gradient change and gradient slope as features |
...
detected it. On this cohort (ideal rung, 1,800 patients, seed 20260917) those intervals have a
median width of 1.1 years and a maximum of 4.2 years, so a deterioration recorded at an examination may have begun four years earlier. Supplying
...
|---|---|---|---|
| patients with any examination | 27% | 52% | 83.8% |
| examinations per patient | 1.00 | 0.85 | 1.62 |
| patients with more than one gradient | 0% | 23% | 16.2% |
| **mortality observed** | **yes** | **no** | **no** |
| patients with an event, per 100 | — | 9.4 | 8.55 |
| event rows per 100 | 20.5 | 15.2 | 8.55 |
...
n = 117. The extract column is measured by `scripts/09_real_extract_rung.py` with the same
`ladder_metrics` code, and printed in [`synthetic/results.md`](synthetic/results.md).

**Measured with the same code, the simulated rung is now too pessimistic.** The corrected rung was
calibrated to a stricter count, post-operative prosthetic examinations in 61 of the 117 patients
(`notebooks/synthetic/degrade.py`). Under the shared definition the extract has an examination
for 83.8% of patients and 1.62 each, against 52% and 0.85 in the simulation, and it records fewer
endpoint rows (8.55 against 15.2 per 100). Affected patients agree (8.55 against 9.4). The
34% and 42% losses below are therefore measured against a rung that sees less surveillance than
the extract does, and recalibrating that rung to the shared definition is open.
...
to stage 3, then is reintervened is one affected patient and three endpoint rows. On the extract the two counts
coincide, 8.55 per 100 each, because no valve episode there has more than one structural row.
Every event count this repository quotes for the extract, and the unit each one counts, is
tabulated in
