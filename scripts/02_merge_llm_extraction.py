...
def main():
    require(
        JSON_DIR,
        hint=(
            "The per-note LLM extraction is not regenerated by these scripts; ask whoever ran\n"
            "the pass for a copy of data/llm_json/. See scripts/README.md."
        ),
    )
    require(OUT, hint="Run scripts/01_extract_rules.py first.")
    docs, bad = load()
    frames = flatten(docs)
    x = pd.ExcelFile(OUT)
